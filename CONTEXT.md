...
A retrospective, batch-computed allocation over a case set already fully observed, used only as an evaluation baseline for how close the live streaming allocator came to the best possible outcome — never presented as a decision the agent itself made, since the agent never had that foresight.
