"""Ticket 20: a `subscription.halted` webhook that arrives while the app is
wired to real Razorpay (`GATEWAY_BACKEND=razorpay`) creates a `source=REAL`
case, so it is excluded from the Decision Engine's posterior (ticket 07's
exclusion rule). Under the fake gateway (dev, demo, tests) an inbound webhook
is synthetic and the case stays SIMULATED -- the pre-ticket-20 behaviour,
unchanged.
"""
