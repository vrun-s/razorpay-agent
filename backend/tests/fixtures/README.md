...
ingestion path (`webhooks._extract_halted_subscription` -> `create_case_from_halted_subscription`
-> one decision cycle -> event-id dedupe).

How to capture it:

1. Run the local backend against real test mode + the `ssh -R` tunnel (see
   `docs/ticket-20-halted-slice-runbook.md`).
2. Either read the exact request body from the Razorpay dashboard's webhook
   delivery log, or point the tunnel at `test-scripts/capture_halted_webhook.py`
   first, which writes `test-scripts/captured/subscription-halted-<ts>.json`
   (the `body_json` field there is what goes in this file).
